        ...
        for (var j = 0; j < nodes.length; j++) {
          if (nodes[i].port != nodes[j].port && nodes[i].zone != 'Unknown') {
            ...
            if (nodes[j].zone == 'Unknown') {
              ...
              command = 'sudo tcset --device em1 --delay ' + Math.floor(lat[0][nodes[j].zone]) + ' --src-port ' + nodes[i].port + ' --dst-port ' + nodes[j].port + ' --delay-distro 20'
              run(command).then(() => console.log(i))
            }
          } else {
            let a = Math.floor(Math.random() * (500 - 0) + 0)
            let command = 'sudo tcset --device em1 --delay ' + a + ' --src-port ' + nodes[i].port + ' --dst-port ' + nodes[j].port + ' --delay-distro 20'
            run(command).then(() => console.log(i))
          }
        }
      }
